...
namespace Monaverse.Examples
{
    public class MonaCollectibleItemExample : MonoBehaviour
    {
        ...
        {
            //get url from glb here
            //print all the token object variables
            // foreach (var prop in collectible.GetType().GetProperties())
            // {
            //     Debug.Log(prop.Name + " = " + prop.GetValue(collectible, null));
            // }
            _onClick = onClick;
